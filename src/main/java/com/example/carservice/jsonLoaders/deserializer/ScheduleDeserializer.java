...
public class ScheduleDeserializer implements JsonDeserializer<MaintenanceSchedule> {
  ...
  @Override
  public MaintenanceSchedule deserialize(
      JsonElement jsonElement, Type type, JsonDeserializationContext jsonDeserializationContext)
      throws JsonParseException {
    JsonObject jsonObject = jsonElement.getAsJsonObject();
    var schedule = new MaintenanceSchedule();
    schedule.setMaintenanceDate(
        Utils.parseDate(jsonObject.getAsJsonPrimitive("maintenanceDate").getAsString()));
    schedule.setMaintenanceTime(
        TimeWindows.valueOf(jsonObject.getAsJsonPrimitive("maintenanceTime").getAsString()));
    if (jsonObject.has("technicalInspection")) {
      String technicalInspectionName =
          jsonObject.getAsJsonPrimitive("technicalInspection").getAsString();
      schedule.setTechnicalInspection(
          Utils.getTechnicalInspectionWithName(technicalInspectionName));
    }
