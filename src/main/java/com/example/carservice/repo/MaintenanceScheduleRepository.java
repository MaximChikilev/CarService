...
public interface MaintenanceScheduleRepository extends JpaRepository<MaintenanceSchedule, Long> {
    ...
    MaintenanceSchedule findByMaintenanceDateAndAndMaintenanceTime(Date startDate, TimeWindows timeWindow);

    @Query("SELECT x FROM MaintenanceSchedule x WHERE x.car.licensePlateNumber=:lpn AND x.maintenanceDate>=:date")
    List<MaintenanceSchedule> findByLicensePlateNumberAndDate(@Param("lpn") String lpn, @Param("date") Date date);
}
