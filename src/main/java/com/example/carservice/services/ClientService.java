    ...
    public List<ClientCarRecommendedToServiceDTO> getClientsCarsRecommendedToService() {
        ...
        for (ClientCarAVGMileageDTO clientCarAVGMileage : clientCarAVGMileageList) {
            ...
            for (InspectionDTO inspection : inspections) {
                int mileageToThisInspection = inspection.getMileageToPass() - (clientCarAVGMileage.getMileage() - numberOfCompleteServiceCycles * maxMileageToPass);
                if ((mileageToThisInspection > 0) && (mileageToThisInspection / clientCarAVGMileage.getAvgMileage() < daysBeforeService)) {
                    var clientCarRecommendedToService = new ClientCarRecommendedToServiceDTO(clientCarAVGMileage, inspection.getInspectionName());
