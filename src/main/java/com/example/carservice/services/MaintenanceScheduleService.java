    ...
    public FreeDataTimeForInspectionDTO getFreeMaintenanceSchedulesUntilDate(Date endDate) {
        ...
        do {
            for (TimeWindows timeWindow : timeWindows) {
                if (((MaintenanceScheduleRepository) repository).findByMaintenanceDateAndAndMaintenanceTime(calendar.getTime(), timeWindow) == null)
                    freeDataTimes.add(dateFormat.format(calendar.getTime()) + "(" + timeWindow.getHours() + ")");
