...
public class SparePartService extends EntityService<SparePart> {
    ...
    protected SparePartService(SparePartRepository repository, SparePartJsonManager sparePartJsonManager, ManufacturerService manufacturerService) {
        super(repository);
    ...
    protected List<SparePart> loadEntityListFromJson() throws IOException {
        ...
        for (SparePart sparePart : spareParts) {
            if (sparePart.getManufacturer() != null) {
                sparePart.setManufacturer(manufacturerService.getByName(sparePart.getManufacturer().getName()));
            }
        }
        return spareParts;
    }
    public List<SparePart> replacingObjectsAfterDeserializationWithReal(List<SparePart> inputList){
        ...
        for (SparePart sparePart:inputList){
            var realSparePart = ((SparePartRepository) repository).findByPartNumber(sparePart.getPartNumber());
